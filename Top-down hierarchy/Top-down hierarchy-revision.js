...
function checkValues(objectArr, counterB, counterC) {
  ...
  var currentManagerSysID = objectArr[counterB].parent;
  var sysUser = new GlideRecord('sys_user');
  ...
  /*
    The main logic of this function works by first passing in a parent from the array
    ...
    Once the function has gone through each parent in the array, it will stop the recursion and return the results.
  */
  ...
  if (objectArr[counterB].parent === objectArr[counterC].sys_id) {
    return checkValues(objectArr, counterB + 1, 0)
  }
...
unsortedArr.forEach(function(element){
     element.children = [];
})
var sortedArr = unsortedArr.filter(function(element){
    if(element.parent == value){

    }
    
},unsortedArr.forEach(function(value){
    
})) 


stringifyOutput(unsortedArr)
